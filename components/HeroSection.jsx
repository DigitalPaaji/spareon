...
// Define our slide data based exactly on the image provided
const slideData = [
  {
    id: 1,
    line1: "PRECISION PARTS.",
  ...
  useEffect(() => {
    let ctx = gsap.context(() => {
      ...
      tl.from(".gsap-text", {
        ...
        opacity: 0,
        duration: 0.8,
        stagger: 0.15, // Slightly increased stagger for the stacked text
        ease: "power3.out",
      })
      // Image slide-in
      .from(
    ...
    <section className="relative w-full min-h-screen flex items-center pt-24 pb-12 overflow-hidden z-0">
      ...
      >
        ...
        <div className="w-full lg:w-[55%] pl-4 md:pl-12 lg:pl-24 xl:pl-40 flex flex-col justify-center text-left">
          ...
          {/* Top Line */}
          <h2 className={`gsap-text text-2xl md:text-3xl lg:text-[2.5rem] font-black uppercase tracking-wide mb-1 ${slide.line1Color}`}>
            {slide.line1}
          </h2>
          
          {/* Main Bottom Line */}
          <h1 className={`gsap-text text-4xl md:text-5xl lg:text-6xl font-black uppercase tracking-tight mb-6 leading-none ${slide.line2Color}`}>
            {slide.line2}
          </h1>

          {/* Description */}
          <p className="gsap-text text-gray-200 text-base md:text-lg font-normal mb-8 max-w-lg leading-relaxed">
            {slide.description}
          </p>
...
<div className="w-fit gsap-text">
  ...
  >
    ...
    <span className="relative z-10 flex h-8 w-8 items-center justify-center rounded-full bg-white transition-transform duration-300 group-hover:translate-x-0.5">
      ...
      <FaPlay className="text-[10px] text-[#107BEB] ml-0.5" />
    </span>
  </Link>
</div>
