...
const AboutusSection = () => {
  ...
  return (
    <section
      ref={containerRef}
      className="overflow-hidden px-4 py-24 md:px-12 lg:px-24 xl:px-40"
    >
      <div className="grid items-center gap-10 lg:grid-cols-2 lg:gap-16">
        {/* Left Column */}
        <div className="flex flex-col items-start pr-0 lg:pr-8">
          <h2 className="gsap-fade-up mb-4 text-[26px] font-bold uppercase tracking-wide text-[#0B1A30] md:text-[32px]">
            WELCOME TO{" "}
            <span className="text-[#1762A7]">SPAREON INDIA</span>
          </h2>

          <p className="gsap-fade-up mb-8 text-[15px] leading-[1.8] text-gray-600 md:text-base">
            We are the authorized channel partner of Taiwan-based Spareon
            Taiwan, a renowned manufacturer of high-quality spare parts for
            advanced rice milling machinery. Our products are engineered with
            precision, manufactured in Taiwan, and trusted by rice millers
            across India.
          </p>
          ...
          <div className="gsap-fade-up w-fit">
            <Link
              href="/about"
              className="group relative flex items-center gap-4 overflow-hidden rounded-r-full bg-gradient-to-r from-[#056483] to-[#063B73] py-1.5 pl-6 pr-1.5 text-sm font-bold text-white shadow-[0_10px_25px_rgba(16,123,235,0.3)] transition-all duration-300 hover:shadow-[0_15px_30px_rgba(16,123,235,0.45)]"
            >
              <span className="absolute inset-0 -translate-x-[105%] bg-gradient-to-r from-[#063B73] to-[#056483] transition-transform duration-500 group-hover:translate-x-0" />
              ...
              <span className="relative z-10 flex h-8 w-8 items-center justify-center rounded-full bg-white transition-transform duration-300 group-hover:translate-x-0.5">
                <FaArrowRight className="ml-0.5 text-[10px] text-[#107BEB]" />
              </span>
            </Link>
          </div>
        </div>

        {/* Right Column: Local Autoplay Video */}
        <div className="gsap-fade-left group relative h-[280px] w-full overflow-hidden rounded-md bg-[#0A162B] shadow-2xl md:h-[400px]">
          <video
            ...
            preload="metadata"
            poster="/about.webp"
            aria-label="Spareon India introduction video"
          >
            {/* WebM loads faster when available */}
            <source
              src="/spareon-intro.mp4"
              type="video/webm"
            />
          ...
          {/* Subtle overlay */}
          <div className="pointer-events-none absolute inset-0 bg-gradient-to-t from-[#0A162B]/25 via-transparent to-transparent" />
        </div>
      </div>
    </section>
  );
